...
class VideoPlayerResizeDelegate(private val player: VideoPlayerActivity) {
    ...
    fun setVideoScale(scale: MediaPlayer.ScaleType) = player.service?.run {
        ...
        var msgSize = ""
        if (scale === MediaPlayer.ScaleType.SURFACE_FILL) {
            msgSize = "화면 전체 크기"
        } else if (scale === MediaPlayer.ScaleType.SURFACE_16_9) {
            msgSize = "16 : 9"
        } else if (scale === MediaPlayer.ScaleType.SURFACE_4_3) {
            msgSize = "4 : 3"
        } else if (scale === MediaPlayer.ScaleType.SURFACE_ORIGINAL) {
            msgSize = "원본 크기"
        }
        player.showMsg(msgSize)

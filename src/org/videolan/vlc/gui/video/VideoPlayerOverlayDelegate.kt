...
class VideoPlayerOverlayDelegate (private val player: VideoPlayerActivity) {
    var _bt_exit: Button? = null

    var mVideoInfo: TextView? = null
    var _tv_video_err:TextView? = null

    var _rl_time_select: RelativeLayout? = null
    var _tv_time_hour_1: TextView? = null
    var _tv_time_hour_2: TextView? = null
    var _tv_time_min_1: TextView? = null
    var _tv_time_min_2: TextView? = null
    var _tv_time_sec_1: TextView? = null
    var _tv_time_sec_2: TextView? = null
    var _tv_video_rate: TextView? = null
    ...
    fun showChangeSpeedMsg(msg: String) {
        _tv_video_rate!!.text = msg
        _tv_video_rate.setVisibility(View.VISIBLE)
    }

    fun showTimeSelect(isShow: Boolean) {
        if (isShow) {
            mVideoInfo!!.text =
                Tools.millisToString(player.service?.getTime()!!) + " / " + Tools.millisToString(player.service?.length!!)
            mVideoInfo.setVisibility(View.VISIBLE)
            _rl_time_select.setVisibility(View.VISIBLE)
        } else {
            _rl_time_select.setVisibility(View.GONE)
